    ...
    def __init__(self, base_name, base_path=''):
        ...
        if '7666' in base_name:
            ...
            for file_name in file_names:
                ...
                for line in raw_data:
                    try:
                        header_search = re.search(
                            'THRUST COEFFICIENT', line[0])
                        if header_search:
                            break
                ...
                while True:
                    ...
                    try:
                        value_line_search = re.match(
                            r'[0\.\d+,CP]', value_line[0])
                    except IndexError:
            ...
            # wipe temp storage, to be sure to start with a clean slate for the
            # next type of data
            temp_data_storage1 = []
            temp_data_storage2 = []
            temp_data_storage3 = []
...
#            file_glob = base_path + '*' + base_name + '*.csv'
            ...
            for file_name in file_names:
                ...
                header_lines = 2
                line1 = raw_data.next()
                model_match = re.search(r'(\S+)\s+(\S+)\s.*', line1[0])
                ...
                self.dia = float(
                    re.search(
                        r'[^-]+-[^-]+-[^-]+-(\d+).*',
                        line1[0]).group(1))
                self.dia = U.length_conv(
                    self.dia, from_units='cm', to_units='in')
...
def ct2thrust(Ct, Rho, rpm, dia, thrust_units='lb',
              density_units='lb/ft**3', dia_units='in'):
    """
...
def eff2thrust(eff, bhp, TAS, power_units='hp',
               speed_units='kt', thrust_units='lb'):
    """
...
def tip_mach(tas, rpm, temperature, dia, speed_units='kt',
             temp_units='C', dia_units='in'):
    """
    ...
    dia = U.length_conv(dia, from_units=dia_units, to_units='m')
    tas = U.speed_conv(tas, from_units=speed_units, to_units='m/s')
    speed_of_sound = SA.temp2speed_of_sound(
        temperature, temp_units=temp_units, speed_units='m/s')
...
def prop_eff(prop, bhp, rpm, tas, altitude, temp='std', power_units='hp',
             alt_units='ft', temp_units='C', speed_units='kt', dia_units='in'):
    """
    ...
    if temp == 'std':
        temp = SA.alt2temp(
            ...
            alt_units=alt_units)
    temp_ratio = U.temp_conv(
        temp,
        from_units=temp_units,
        to_units='K') / 288.15
...
def prop_eff2(prop, thp, rpm, tas, altitude, temp='std', power_units='hp',
              alt_units='ft', temp_units='C', speed_units='kt', dia_units='in'):
    """
    ...
    for n in range(100):
        ...
        prop_eff_guess_new = prop_eff(
            ...
            speed_units,
            dia_units)
        if (np.fabs((prop_eff_guess_new - prop_eff_guess) / prop_eff_guess) < 0.0001):
    ...
    # convergence was not achieved before 100 iterations.  Return the average
    # of the current and previous guesses
...
def prop_data(prop, bhp, rpm, tas, altitude, temp='std', power_units='hp',
              alt_units='ft', temp_units='C', speed_units='kt', dia_units='in', thrust_units='lb'):
    """
...
def cp2ct(prop, Cp, J, tip_mach):
    ...
    """
    mach_low = max(prop.Ct_mach_min, prop.prop_CT_map[max(min(N.searchsorted(
        prop.prop_CT_map[:, 0, 0], tip_mach) - 1, prop.z_size - 1), 0), 0, 0])
    if mach_low == prop.Ct_mach_max:
        mach_low = prop.prop_CT_map[-2, 0, 0]
    mach_high = min(prop.Ct_mach_max, prop.prop_CT_map[min(N.searchsorted(
        prop.prop_CT_map[:, 0, 0], tip_mach), prop.z_size - 1), 0, 0])
    if mach_high == prop.Ct_mach_min:
        mach_high = prop.prop_CT_map[1, 0, 0]

    J_low = max(prop.Ct_J_min, prop.prop_CT_map[0, 0, max(
        min(N.searchsorted(prop.prop_CT_map[0, 0, 1:], J), prop.x_size - 2), 1)])
    if J_low == prop.Ct_J_max:
        J_low = prop.prop_CT_map[0, 0, -2]
    J_high = min(prop.Ct_J_max, prop.prop_CT_map[0, 0, min(
        N.searchsorted(prop.prop_CT_map[0, 0, 1:], J), prop.x_size - 2) + 1])
    if J_high == prop.Ct_J_min:
        J_high = prop.prop_CT_map[0, 0, 2]

    Cp_low = max(prop.Ct_Cp_min, prop.prop_CT_map[0, max(
        min(N.searchsorted(prop.prop_CT_map[0, 1:, 0], Cp), prop.y_size - 2), 1), 0])
    if Cp_low == prop.Ct_Cp_max:
        Cp_low = prop.prop_CT_map[0, -2, 0]
    Cp_high = min(prop.Ct_Cp_max, prop.prop_CT_map[0, min(
        N.searchsorted(prop.prop_CT_map[0, 1:, 0], Cp), prop.y_size - 2) + 1, 0])
...
def cp2ct_alt(prop, Cp, bhp, rpm, tas, altitude, temp='std', power_units='hp',
              alt_units='ft', temp_units='C', speed_units='kt', dia_units='in'):
    """
...
def cp2eff(prop, Cp, J, tip_mach):
    ...
    (J) and the mach number of the blade tip.
    """
    mach_low = max(prop.eff_mach_min, prop.prop_eff_map[max(min(N.searchsorted(
        prop.prop_eff_map[:, 0, 0], tip_mach) - 1, prop.z_size - 1), 0), 0, 0])
    if mach_low == prop.eff_mach_max:
        mach_low = prop.prop_eff_map[-2, 0, 0]
    mach_high = min(prop.eff_mach_max, prop.prop_eff_map[min(
        N.searchsorted(prop.prop_eff_map[:, 0, 0], tip_mach), prop.z_size - 1), 0, 0])
    if mach_high == prop.eff_mach_min:
        mach_high = prop.prop_eff_map[1, 0, 0]

    J_low = max(prop.eff_J_min, prop.prop_eff_map[0, 0, max(
        min(N.searchsorted(prop.prop_eff_map[0, 0, 1:], J), prop.x_size - 2), 1)])
    if J_low == prop.eff_J_max:
        J_low = prop.prop_eff_map[0, 0, -2]
    J_high = min(prop.eff_J_max, prop.prop_eff_map[0, 0, min(
        N.searchsorted(prop.prop_eff_map[0, 0, 1:], J), prop.x_size - 2) + 1])
    if J_high == prop.eff_J_min:
        J_high = prop.prop_eff_map[0, 0, 2]
    if J_high == J_low:
        J_high = J_low + 0.1

    Cp_low = max(prop.eff_Cp_min, prop.prop_eff_map[0, max(
        min(N.searchsorted(prop.prop_eff_map[0, 1:, 0], Cp), prop.y_size - 1), 1), 0])
    if Cp_low == prop.eff_Cp_max:
        # print('in CP-low')
        Cp_low = prop.prop_eff_map[0, -2, 0]
    Cp_high = min(prop.eff_Cp_max, prop.prop_eff_map[0, min(
        N.searchsorted(prop.prop_eff_map[0, 1:, 0], Cp), prop.y_size - 1) + 1, 0])
...
def cp2blade_angle(prop, Cp, J, tip_mach):
    ...
    (J) and the mach number of the blade tip.
    """
    mach_low = max(prop.blade_angle_mach_min, prop.blade_angle_map[max(min(N.searchsorted(
        prop.blade_angle_map[:, 0, 0], tip_mach) - 1, prop.z_size - 1), 0), 0, 0])
    if mach_low == prop.blade_angle_mach_max:
        mach_low = prop.blade_angle_map[-2, 0, 0]
    mach_high = min(prop.blade_angle_mach_max, prop.blade_angle_map[min(
        N.searchsorted(prop.blade_angle_map[:, 0, 0], tip_mach), prop.z_size - 1), 0, 0])
    if mach_high == prop.blade_angle_mach_min:
        mach_high = prop.blade_angle_map[1, 0, 0]

    J_low = max(prop.blade_angle_J_min, prop.blade_angle_map[0, 0, max(
        min(N.searchsorted(prop.blade_angle_map[0, 0, 1:], J), prop.x_size - 2), 1)])
    if J_low == prop.blade_angle_J_max:
        J_low = prop.blade_angle_map[0, 0, -2]
    J_high = min(prop.blade_angle_J_max, prop.blade_angle_map[0, 0, min(
        N.searchsorted(prop.blade_angle_map[0, 0, 1:], J), prop.x_size - 2) + 1])
    if J_high == prop.blade_angle_J_min:
        J_high = prop.blade_angle_map[0, 0, 2]

    Cp_low = max(prop.blade_angle_Cp_min, prop.blade_angle_map[0, max(
        min(N.searchsorted(prop.blade_angle_map[0, 1:, 0], Cp), prop.y_size - 1), 1), 0])
    if Cp_low == prop.blade_angle_Cp_max:
        # print('in CP-low')
        Cp_low = prop.blade_angle_map[0, -2, 0]
    Cp_high = min(prop.blade_angle_Cp_max, prop.blade_angle_map[0, min(
        N.searchsorted(prop.blade_angle_map[0, 1:, 0], Cp), prop.y_size - 1) + 1, 0])
...
def blade_angle2bhp(prop, blade_angle, rpm, tas, altitude, temp='std', power_units='hp',
                    alt_units='ft', temp_units='C', speed_units='kt', dia_units='in'):
    """
